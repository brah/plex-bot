...
class MediaCommands(commands.Cog):
    ...
    async def random(self, ctx, *args):
        ...
        try:
            ...
            view = RandomMediaView(ctx, items, self.tautulli.tautulli_ip, self.plex_embed_color, use_https=self.tautulli.use_https, api_key=self.tautulli.api_key)
    ...
    async def _build_media_embed(self, metadata: dict, user_stats_resp, watch_time_resp):
        ...
        if thumb:
            file, url = await prepare_thumbnail_for_embed(self.tautulli.tautulli_ip, thumb, use_https=self.tautulli.use_https, api_key=self.tautulli.api_key)
            if file and url:
                embed.set_thumbnail(url=url)
