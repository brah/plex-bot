...
class Visualizations(commands.Cog):
    ...
    async def generate_hour_chart(self, hour_counts: pd.Series, days: int, user_name: str = None) -> Optional[BytesIO]:
        """
        ...
        plt.savefig(image_stream, format="png", facecolor=plt.gcf().get_facecolor(), dpi=BotConfig.CHART_DPI)
        plt.close()
        image_stream.seek(0)
        return image_stream

    async def generate_day_chart(self, day_counts: pd.Series, days: int, user_name: str = None) -> Optional[BytesIO]:
        """
    ...
    async def generate_media_type_by_day_chart(self, media_type_data: pd.DataFrame, days: int, user_name: str = None) -> Optional[BytesIO]:
        """
        ...
        try:
            media_type_pivot = media_type_data.pivot(index="date", columns="media_type", values="count").fillna(0)
    ...
    async def generate_play_count_by_month_chart(self, month_counts: pd.Series, days: int, user_name: str = None) -> Optional[BytesIO]:
        """
